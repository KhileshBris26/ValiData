...
const Dashboard: React.FC = () => {
  ...
  const refreshDashboard = async () => {
    ...
    try {
      ...
      const [analyticsRes, logsRes, lineageRes, metricsRes, rulesRes, anomaliesRes] = await Promise.all([
        ...
        axios.get(`${API_BASE}/dashboard/metrics?platform=${platform}`),
        axios.get(`${API_BASE}/dashboard/rules?platform=${platform}`),
        axios.get(`${API_BASE}/dashboard/anomalies?platform=${platform}`)
      ]);
  ...
  const syncAllRulesToBackend = async () => {
    try {
      // 1. Fetch backend rules first to avoid overwriting database with client states
      ...
      if (backendRules.length > 0) {
        ...
        backendRules.forEach((br: any) => {
          ...
          rulesByKey[key].push({
            ...
            name: br.rule_type,
            rule_params: typeof br.rule_params === 'string' ? JSON.parse(br.rule_params) : br.rule_params,
            status: br.status === 'Active' ? 'active' : 'deactivated'
          });
        });

        // Set key-by-key in localStorage
        Object.entries(rulesByKey).forEach(([key, val]) => {
          localStorage.setItem(key, JSON.stringify(val));
        });
      }

      // 4. Scan localStorage and send all local rules to backend
