...
# I'll just use split
parts = content.split('@app.post("/api/v1/metadata/save")')
if len(parts) > 1:
    before = parts[0]
    after_parts = parts[1].split('@app.post("/api/v1/metadata/entities")')
    if len(after_parts) > 1:
        after = '@app.post("/api/v1/metadata/entities")' + after_parts[1]
    else:
        print("Could not find entities")
        exit(1)
        
    replacement = """@app.post("/api/v1/metadata/save")
